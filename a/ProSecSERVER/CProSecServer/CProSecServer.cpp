...
//
#include "CProSecServer.hpp"





void CProSecServer::ClientConnectionHandler()
{
    int i = 0;
    while (true) 
    {
        ...
        //clientThreads.emplace_back(&CProSecServer::ClientHandler, std::ref(connectedClients[i]), std::ref(connectedClients));
        clientThreads.emplace_back(&CProSecServer::ClientHandler, this, std::ref(connectedClients[i]));        
        i++;
...
/*
void CProSecServer::joinClientThreads()
{
...
*/
void CProSecServer::StartServer()
{
    std::cout << "[CProSecServer Start]: Server Started! " << std::endl;
    try
    {
        std::cout << "[ProSec Server]: Server is running. Waiting for connections..." << std::endl;
        
        std::thread ConnectionHandler(&CProSecServer::ClientConnectionHandler, this);
        ConnectionHandler.join();
    }
    catch (const std::exception& ShitCode)
    {
        setlocale(LC_ALL, "rus");
        std::cout << "[ProSec Server ShitCode]: Error: " << ShitCode.what() << std::endl;
    }
    
    std::cout << "[CProSecServer Start]: Called Exit. " << std::endl;
    std::this_thread::sleep_for(std::chrono::milliseconds(1000));
}

void CProSecServer::ClientHandler(Client& client)
{
    try
    {
        while (true)
        {
            ...
            {
                std::cout << "[ProSec Server ShitCode]: End of filestream error" << std::endl;
                break;
            }
    ...
    {
        setlocale(LC_ALL, "rus");
        std::cerr << "[ProSec Server ShitCode]: Error: " << ShitCode.what() << std::endl;
    }
}



std::string CProSecServer::reciveMessageFromSocket(boost::asio::ip::tcp::socket& socket)
{
...
bool CProSecServer::messageHandler(std::string message, Client& client)
{
...
bool CProSecServer::auth(boost::asio::ip::tcp::socket& socket_, std::string& message, Client& client)
{
...
bool CProSecServer::sendMessageToClient(std::string& message, Client& sender)
{
    std::string UID;
    Client* reciever;
    if (findClient(message.substr(0, message.find('\t'))))   //если клиент найдётся, указатель будет не nullptr => true 
    {
        ...
        //данные отправителя и получателе теперь хранятся в переменных sender
        //и *reciever
        std::thread th(&CProSecServer::messageSaver, this, std::ref(message), std::ref(sender), std::ref(*reciever)); //сохранение сообщения в файл
        th.detach();

        message.insert(0, m_MessageKey + sender.clientID);
        sendReplyToClient(reciever->socket, message);
    }

    else sendReplyToClient(sender.socket, "Reciever is offline\n");
    return false;
}

bool CProSecServer::messageSaver(std::string message, Client& client_sender, Client& client_reciever)
{
    std::string filename = "Dialogs\\";
    
    if (client_sender.clientID > client_reciever.clientID) 
        filename = filename + client_reciever.clientID + "_" + client_sender.clientID + ".txt";
    else filename = filename + client_sender.clientID + "_" + client_reciever.clientID + ".txt";
   
    FileMutexes *file_mutex;
    file_mutex = findMutex(filename);
    if(file_mutex == nullptr)
    {
        file_mutex->filename = filename;
        file_mutex->mtx = new std::mutex;
        ExistingMutexes.push_back(*file_mutex);
    }
    
    std::lock_guard<std::mutex> lock(*file_mutex->mtx);
    try 
    {
        std::fstream dialog;
        dialog.open(file_mutex->filename, std::ios::in | std::ios::out);
        
        if (dialog.is_open())
        {
            message = client_sender.clientID + "\t" + message;

            dialog.seekp(0, std::ios::beg);
            dialog.write(message.c_str(), message.size());
    ...
    {
    std::cout << "Ошибка при записи в файл: " << ex.what();
    return false;
    }
}
...
CProSecServer::Client* CProSecServer::findClient(std::string clientID)
{
...
CProSecServer::FileMutexes* CProSecServer::findMutex(std::string filename)
{
    for (int i = 0; i <ExistingMutexes.size(); i++)
        if (ExistingMutexes[i].filename == filename) return &ExistingMutexes[i];
    return nullptr;
}

bool CProSecServer::createClient(unsigned clientID, std::string login, std::string password, std::string name)
{
...
void  CProSecServer::sendReplyToClient(boost::asio::ip::tcp::socket& socket, const std::string& message)
{
    const std::string msg = message + "\n";
    boost::asio::write(socket, boost::asio::buffer(message));
}

CProSecServer::~CProSecServer()
{
